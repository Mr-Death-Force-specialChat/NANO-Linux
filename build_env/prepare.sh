...
if ! test $(id -u builderNL) ; then

groupadd builderNL
useradd -s /bin/bash -g builderNL -m -k /dev/null builderNL
echo "enter builders password:>"
passwd builderNL
chown -v builderNL $LFS/{usr,lib,var,etc,bin,sbin,tools,lib64}

dbhome=$(eval echo "~builderNL")
